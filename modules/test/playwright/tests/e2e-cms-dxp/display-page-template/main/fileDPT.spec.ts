...
test(
	'A CMS image file is reachable at its DPT friendly URL for GUEST',
	{tag: ['@LPD-85683', '@LPD-85683/TC-1.d']},
	async ({
		apiHelpers,
		browser,
		...
		const friendlyUrlPath = `tc1d-${getRandomString()}`.toLowerCase();
		...
		await test.step('GUEST reaches the image at its DPT friendly URL', async () => {
