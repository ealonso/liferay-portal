...
<aui:script>
	...
	function <portlet:namespace />toggleFixedIssuesField() {
		...
		var dxp70AndNewerPatcherProductVersionIds =
			${dxp70AndNewerPatcherProductVersionIdsJSONArray};
		...
		if (
			dxp70AndNewerPatcherProductVersionIds.indexOf(
				Number(patcherProductVersionId)
			) < 0
		) {
	...
	function <portlet:namespace />toggleHideCheckbox() {
		...
		var combined = A.one('#<portlet:namespace />combinedBranchCheckbox').attr(
			'checked'
		);

		var marketplaceReleasePatcherProductVersionIds =
			${marketplaceReleasePatcherProductVersionIds};

		if (
			marketplaceReleasePatcherProductVersionIds.includes(
				parseInt(productVersionId)
			) &&
			!combined
		) {
