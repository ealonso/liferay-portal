...
<liferay-ui:search-container emptyResultsMessage="there-are-no-accounts" iteratorURL="${alloySearchResult.portletURL}">
			...
			<table class="account-table">
				<c:forEach items="${patcherProductVersions}" var="patcherProductVersion">
					<tr>
						...
						<td class="slim">
							<fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" timeZone="${timeZone}" value="${patcherBuild.statusDate}" var="statusDataDate" />
							<fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss z" timeZone="${timeZone}" value="${patcherBuild.statusDate}" var="statusDisplayDate" />
	...
	Liferay.on('allPortletsReady', function (event) {
		...
		AUI().use(
			...
			function (A) {
				...
				var patcherAccountsJSONURL = Liferay.PortletURL.createURL(
					'${viewPatcherAccountsURL}.json'
				);
				...
				var autoComplete = new A.AutoCompleteList({
					...
					source: function () {
						var inputValue = A.one(
							'#<portlet:namespace />accountEntryCode'
						).get('value');
									...
									success: function () {
										...
										data.data.forEach(function (d) {
											if (results.length < 20) {
												results.push({
													raw: d,
													display: d.accountEntryCode,
													text: d.accountEntryCode,
												});
											}
										});
				...
				autoComplete.on('select', function (event) {
					...
					var viewPatcherAccountURL = Liferay.PortletURL.createURL(
						'${viewPatcherAccountURL}'
					);

					viewPatcherAccountURL.setPortletId(
						'${PortletKeys.OSB_PATCHER}'
					);

					viewPatcherAccountURL.setParameter(
						'accountEntryCode',
						event.result.text
					);
