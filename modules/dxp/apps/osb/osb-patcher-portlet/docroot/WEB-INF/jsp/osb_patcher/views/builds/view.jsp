...
<aui:field-wrapper name="createDate">
	<fmt:formatDate type="both" value="${patcherBuild.createDate}" />
</aui:field-wrapper>

<aui:field-wrapper name="modifiedDate">
	<fmt:formatDate type="both" value="${patcherBuild.modifiedDate}" />
</aui:field-wrapper>

<aui:field-wrapper name="statusDate">
	<fmt:formatDate type="both" value="${patcherBuild.statusDate}" />
</aui:field-wrapper>
...
<c:if test="${(patcherBuild.patcherProductVersionId != PatcherProductVersionUtil.getPatcherProductVersionId(PatcherProductVersionConstants.LABEL_PRODUCT_VERSION_PORTAL_6X)) && !patcherBuild.childBuild && !StringUtil.equalsIgnoreCase(patcherBuild.name, patcherBuild.originalName)}">
	<aui:field-wrapper>
		...
		<aui:field-wrapper>
			<liferay-ui:icon image="../api/exception" message="" />
...
<c:if test="${alloySearchResult.size > 1}">
	...
	<liferay-ui:search-container iteratorURL="${alloySearchResult.portletURL}">
		...
		>
			<c:if test="${patcherBuild.patcherBuildId == patcherBuildKeyVersion.patcherBuildId}">
				<liferay-ui:search-container-row-parameter name="className" value="selected" />
			</c:if>
			...
			<liferay-ui:search-container-column-text
				name="modified-date"
			>
				<fmt:formatDate type="both" value="${patcherBuildKeyVersion.modifiedDate}" />
			</liferay-ui:search-container-column-text>
