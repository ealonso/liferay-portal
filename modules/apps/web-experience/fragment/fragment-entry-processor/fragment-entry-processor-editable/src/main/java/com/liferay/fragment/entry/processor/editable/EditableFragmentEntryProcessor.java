	...
	public void validateFragmentEntryHTML(String html) throws PortalException {
		...
		try {
			...
			FragmentEntryContentException exception =
				new FragmentEntryContentException(
					LanguageUtil.get(resourceBundle, "invalid-fragment-html"),
					de);
		...
		if (!nonUniqueIds.isEmpty()) {
			throw new FragmentEntryContentException(
				LanguageUtil.get(
					resourceBundle, "element-ids-should-be-unique"));
		}
		...
		if (!editableNodes.stream().allMatch(
			...
			throw new FragmentEntryContentException(
				LanguageUtil.get(
					resourceBundle, "each-editable-element-should-have-an-id"));
		}
	}
