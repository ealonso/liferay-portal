...
package com.liferay.fragment.processor.editable;
...
public class EditableUniqueIdFragmentEntryProcessor
	...
	public boolean validateFragmentEntryHtml(String html)
		...
		try {
			...
			FragmentEntryContentException exception =
				new FragmentEntryContentException("invalid-fragment-html", de);
		...
		return editableNodes.stream().allMatch(
			node -> _validateElements(((Element)node).elements()));
	}

	private boolean _validateElements(List<Element> elements) {
		for (Element element : elements) {
			if (Validator.isNull(element.attributeValue("id"))) {
				return false;
			}

			boolean childNodesHaveId = _validateElements(element.elements());
