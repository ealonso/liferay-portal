...
<liferay-ui:icon-menu direction="left-side" icon="<%= StringPool.BLANK %>" markupView="lexicon" message="<%= StringPool.BLANK %>" showWhenSingleIcon="<%= true %>">
	<c:choose>
		<c:when test='<%= tabs1.equals("my-sites") %>'>
			<c:if test="<%= (group.getPublicLayoutsPageCount() > 0) && mySitesDisplayContext.isDefaultLayoutVisible(group, false) %>">
				<liferay-ui:icon
			...
			<c:if test="<%= (group.getPrivateLayoutsPageCount() > 0) && mySitesDisplayContext.isDefaultLayoutVisible(group, true) %>">
				<liferay-ui:icon
