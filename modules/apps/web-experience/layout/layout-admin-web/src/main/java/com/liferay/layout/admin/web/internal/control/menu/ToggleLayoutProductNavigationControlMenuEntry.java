...
@Component(
	...
	service = {
		...
		ToggleLayoutProductNavigationControlMenuEntry.class
	}
)
public class ToggleLayoutProductNavigationControlMenuEntry
	...
	public boolean includeIcon(
		...
		values.put(
			"iconEdit",
			themeDisplay.getPathThemeImages() + "/lexicon/icons.svg#cog");
		values.put(
			"iconView",
		...
		String redirect = _http.setParameter(
			themeDisplay.getURLCurrent(), "p_p_edit",
			String.valueOf(!editMode));
		...
		if (editMode) {
			try {
				...
				if (selPlid > 0) {
					Layout layout = _layoutLocalService.getLayout(
						ParamUtil.getLong(request, "p_r_p_selPlid"));
			...
			if (layoutPageTemplateEntryId > 0) {
				...
				redirect = _http.setParameter(
					editLayoutURL.toString(), "p_p_edit",
