...
public class FragmentsEditorContext {
	...
	public SoyContext getEditorContext() throws PortalException {
		...
		soyContext.put("defaultLanguageId", _themeDisplay.getLanguageId().replace('_', '-').toLowerCase());
		soyContext.put("languageId", _themeDisplay.getLanguageId().replace('_', '-').toLowerCase());

		String[] langIds = LocaleUtil.toLanguageIds(
			LanguageUtil.getAvailableLocales(_themeDisplay.getSiteGroupId()));

		List<String> languageIds = new ArrayList<String>();

		for (String langId : langIds) {
			languageIds.add(langId.replace('_', '-').toLowerCase());
		}

		soyContext.put("languageIds", languageIds);
