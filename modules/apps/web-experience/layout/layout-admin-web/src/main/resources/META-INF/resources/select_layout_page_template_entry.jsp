...
<%
SelectLayoutPageTemplateDisplayContext selectLayoutPageTemplateDisplayContext = new SelectLayoutPageTemplateDisplayContext(renderRequest, renderResponse, request);
...
<aui:nav-bar cssClass="collapse-basic-search" markupView="lexicon">
	<aui:nav cssClass="navbar-nav">
		...
		<%
		for (LayoutPageTemplateCollection layoutPageTemplateCollection : selectLayoutPageTemplateDisplayContext.getLayoutPageTemplateCollections()) {
			String addContentLayoutURL = layoutsAdminDisplayContext.getAddContentPageURL(layoutPageTemplateCollection.getLayoutPageTemplateCollectionId());
		%>

			<aui:nav-item href="<%= addContentLayoutURL %>" label="<%= layoutPageTemplateCollection.getName() %>" selected="<%= Objects.equals(selectLayoutPageTemplateDisplayContext.getLayoutPageTemplateCollectionId(), layoutPageTemplateCollection.getLayoutPageTemplateCollectionId()) %>" />
...
<aui:form cssClass="container-fluid-1280" name="fm">
	<liferay-ui:search-container
		...
		searchContainer="<%= selectLayoutPageTemplateDisplayContext.getLayoutPageTemplateEntriesSearchContainer() %>"
	>
