...
<c:choose>
	...
	<c:otherwise>
		...
		if (editJournalDisplayContext.getChangeStructure() && (article != null)) {
		...
		<c:if test="<%= (article != null) && Validator.isNotNull(layoutUuid) %>">
			...
			<c:if test="<%= !article.isNew() && !editJournalDisplayContext.isEditDefaultValues() && Validator.isNotNull(urlViewInContext) %>">
