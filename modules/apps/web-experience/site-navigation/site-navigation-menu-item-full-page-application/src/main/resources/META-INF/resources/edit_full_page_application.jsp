...
String selectedFullApplicationPortlet = typeSettingsProperties.get("fullPageApplicationPortlet");
%>
...
<aui:select label='<%= LanguageUtil.get(resourceBundle, "full-page-application") %>' name="TypeSettingsProperties--fullPageApplicationPortlet--">
	...
	for (Portlet portlet : portlets) {
		boolean selected = StringUtil.equals(selectedFullApplicationPortlet, portlet.getPortletId());
	%>

		<aui:option label="<%= portlet.getDisplayName() %>" selected="<%= selected %>" value="<%= portlet.getPortletId() %>" />
