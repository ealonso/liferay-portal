...
List<AssetEntryAction> actions = assetPublisherDisplayContext.getAssetEntryActions(assetEntry.getClassName());
%>

<c:if test="<%= (editPortletURL != null) || ListUtil.isNotEmpty(actions) %>">
	<div class="pull-right">
		<liferay-ui:icon-menu cssClass="visible-interaction" direction="left-side" icon="<%= StringPool.BLANK %>" markupView="lexicon" message="<%= StringPool.BLANK %>" showWhenSingleIcon="<%= true %>">
			...
			<c:if test="<%= ListUtil.isNotEmpty(actions) %>">

				<%
				for (AssetEntryAction assetEntryAction : actions) {
