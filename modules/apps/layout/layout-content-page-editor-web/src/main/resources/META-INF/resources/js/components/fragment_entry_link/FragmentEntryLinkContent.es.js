...
class FragmentEntryLinkContent extends Component {
	...
	_createEditables() {
		...
		const backgroundImageEditables = [...this.refs.content.querySelectorAll('[data-lfr-background-image-id]')].map(
			element => {
		...
		const editableFields = [...this.refs.content.querySelectorAll('lfr-editable')].map(
			editable => {
				const editableValues = (
