AUI.add(
				...
				prototype: {
					initializer: function(config) {
						...
						var eventHandles = [
							...
							instance.afterHostEvent('render', instance._afterRender, instance),
					...
					_onCheckContentDisplayTreeAppend: function(event) {
						...
						TreeUtil.restoreSelectedNode(host.get('selPlid'), node);

						node.eachChildren(A.bind(TreeUtil.restoreSelectedNode, TreeUtil, host.get('selPlid')));
					},
					...
					_onNodeChildrenChange: function(event) {
						...
						target.eachChildren(A.bind(TreeUtil.restoreSelectedNode, TreeUtil, host.get('selPlid')));
					},
	...
	'',
	{
		requires: ['aui-base', 'aui-io-request', 'liferay-tree-util', 'liferay-store']
	}
);