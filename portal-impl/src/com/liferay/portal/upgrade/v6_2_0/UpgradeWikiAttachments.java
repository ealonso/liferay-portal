...
public class UpgradeWikiAttachments extends UpgradeProcess {
	...
	protected void updatePageAttachment(long resourcePrimKey) throws Exception {
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
			...
			while (rs.next()) {
				...
				long nodeFolderId = getFolder(
					groupId, companyId, userId, userName, createDate,
					repositoryId, repositoryFolderId, String.valueOf(nodeId),
					false);
				...
				for (String attachment : attachments) {
					...
					long fileEntryId = addDLFileEntry(
						groupId, companyId, userId, userName, createDate,
						repositoryId, pageFolderId, name, extension, mimeType,
						title, size);

					addDLFileVersion(
						increment(), groupId, companyId, userId, userName,
						createDate, repositoryId, pageFolderId, fileEntryId,
						extension, mimeType, title, size);

					byte[] bytes = DLStoreUtil.getFileAsBytes(
						companyId, CompanyConstants.SYSTEM, attachment);

					DLStoreUtil.addFile(
						companyId, pageFolderId, name, false, bytes);
	...
	protected void updatePageAttachments() throws Exception {
		...
		try {
			...
			while (rs.next()) {
				...
				updatePageAttachment(resourcePrimKey);
			}
		}
		finally {
			DataAccess.cleanUp(con, ps, rs);
