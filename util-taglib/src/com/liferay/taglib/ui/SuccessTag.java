...
public class SuccessTag extends IncludeTag implements BodyTag {
	...
	public int processEndTag() throws Exception {
		...
		if (_toast) {
			String result = StringUtil.replace(
				_TOAST_TMPL_CONTENT,
				StringPool.POUND, StringPool.POUND, values);

			ScriptTag.doTag(
				null,
				"metal-dom/src/all/dom as dom,clay-alert@2.0.2/lib/ClayToast as ClayToast",
				null,
				result, getBodyContent(), pageContext);
		}
		else {
			String result = StringUtil.replace(
				_SUCCESS_TMPL_CONTENT,
				StringPool.POUND, StringPool.POUND, values);
	...
	private static final String _SUCCESS_TMPL_CONTENT = StringUtil.read(
		SuccessTag.class, "success/success.tmpl");
